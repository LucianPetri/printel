...
description: >
  Phase 8B: Executes test cases using the repository-approved browser tooling,
...
Your goal: execute every test case from `testing/test-cases.md` using the
repository-approved browser tooling, track every bug, auto-fix and retest,
and manage the loop until the feature is ready to ship.

**Execution model:** Prefer the repository's npm entry points when they exist.
Read each test case's steps from `testing/test-cases.md`, execute them with the
approved browser tooling, take evidence, and record PASS / FAIL. The `.spec.ts`
files generated in Phase 8A remain the CI/CD companion artifacts.
...
5. The required browser tooling is available
6. If extension code changed, `npm run build:extensions` has completed successfully
...
🔍 Pre-flight check:
  ...
  Browser tooling available? → verify configured runner
  App reachable?             → open {FRONTEND_URL} and capture evidence
  API reachable?             → probe {API_URL}/health if applicable
  Test credentials?          → Verify testing/env.md is populated
  Extension outputs current? → npm run build:extensions (when workspace code changed)
```

**App reachability check:**
Example when `playwright-cli` is the selected browser tool:
```bash
...
Options:
  1. [FIX] Auto-fix — analyze and fix the issue now
...
Non-browser. Runs via the repository-defined test entry point when available.
No browser or auth setup is required for pure unit coverage.
...
3. Resolve the workspace-scoped command (monorepo mode — see
   ...
   - Prefer repository scripts first (for Printel: `npm run test:unit`)
   - Fall back to the detected runner only when no repo script exists
...
> Execution: repository-approved tooling (step-by-step or scripted)
...
2. **Use the repository-approved execution engine.** Read test case steps,
   translate them to runner actions, and verify from captured evidence.
