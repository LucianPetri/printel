...
export default async function migrate(connection) {
    const existingPage = await select().from('cms_page_description').where('url_key', '=', COOKIE_POLICY_URL_KEY).load(connection);
    if (existingPage) {
        await update('cms_page').given({
            status: 1
        }).where('cms_page_id', '=', existingPage.cms_page_description_cms_page_id).execute(connection);
        await update('cms_page_description').given({
        ...
        }).where('cms_page_description_cms_page_id', '=', existingPage.cms_page_description_cms_page_id).execute(connection);
        return;
    }
    ...
    await insert('cms_page_description').given({
        ...
        meta_title: COOKIE_POLICY_NAME,
        meta_description: 'Detalii despre cookie-urile folosite de Printel și despre gestionarea consimțământului.'
    }).prime('cms_page_description_cms_page_id', page.insertId).execute(connection);
}
