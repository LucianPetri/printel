...
function injectAnafRegistrationMarkup(template: string) {
...
export default () => {
  addProcessor('orderConfirmationEmailData', async (data: Record<string, any>) => {
  ...
  addProcessor<any[]>('orderCollectionFilters', (filters) => [
    ...filters,
    {
      key: 'anaf_status',
      operation: ['eq'],
      callback: (query: any, operation: string, value: string, currentFilters: any[]) => {
        query.leftJoin(
          'order_anaf_compliance',
          'order_anaf_compliance',
          'order_anaf_compliance.order_id',
          '=',
          'order.order_id'
        );
