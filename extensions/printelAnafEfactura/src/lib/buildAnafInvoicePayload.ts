...
const { UblBuilder } = require('@florinszilagyi/anaf-ts-sdk') as typeof import('@florinszilagyi/anaf-ts-sdk');
...
function normalizeVatNumber(value: unknown, fallback = 'RO00000000') {
...
function buildPostalZone(value: unknown) {
  const normalized = String(value ?? '').replace(/[^0-9]/g, '').trim();
  return normalized || '010000';
}

function buildAddress(source: Record<string, any> | null, fallbackStreet: string) {
  return {
...
function buildSupplierAddress(settings: Awaited<ReturnType<typeof getEffectiveAnafSettings>>) {
  return {
    street: normalizeText(
      settings.storeAddress || settings.companyRegisteredOffice,
      settings.companyRegisteredOffice || 'Romania'
    ),
...
export async function loadOrderSnapshot(orderId: number) {
  const orderResult = await pool.query(`SELECT * FROM "order" WHERE order_id = $1 LIMIT 1`, [
    orderId
  ]);
  const order = orderResult.rows[0] as Record<string, any> | undefined;
  if (!order) {
...
export async function buildAnafInvoicePayload(orderId: number): Promise<BuiltAnafInvoicePayload> {
  ...
  const customerName = normalizeText(
    customerAddressSource?.company_name ||
      customerAddressSource?.full_name ||
      snapshot.order.customer_full_name,
    snapshot.order.customer_email || `Customer ${snapshot.order.order_number}`
  );
  const customerCompanyId = normalizeText(
    customerAddressSource?.company_name ||
      customerAddressSource?.vat_number ||
      customerAddressSource?.telephone,
    `CUSTOMER-${snapshot.order.order_number}`
  );
  const payload = {
  ...
  const invoiceXml = builder.generateInvoiceXml({
    ...
    supplier: {
      registrationName: normalizeText(
    ...
    customer: {
      ...
      vatNumber: customerAddressSource?.vat_number
