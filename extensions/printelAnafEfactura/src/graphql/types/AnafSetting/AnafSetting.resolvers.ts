...
export default {
  Setting: {
    ...
    anafEnvironmentLocked: async () =>
      (await getEffectiveAnafSettings()).environmentLocked,
    anafSubmissionMode: async (setting: SettingRow[]) =>
      getStringValue(setting, 'anafSubmissionMode') ??
      (await getEffectiveAnafSettings()).submissionMode,
    ...
    anafCompanyTaxId: async (setting: SettingRow[]) =>
      getStringValue(setting, 'companyTaxId') ?? (await getSetting('companyTaxId', null)),
