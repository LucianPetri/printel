...
export default async function migrate(connection) {
    await execute(connection, `CREATE OR REPLACE FUNCTION printel_mark_anaf_manual_review(p_order_id INT, p_reason TEXT)
     RETURNS VOID
     LANGUAGE plpgsql
     AS $$
     BEGIN
     ...
     END;
     $$;`);
    await execute(connection, `CREATE OR REPLACE FUNCTION printel_anaf_handle_order_changes()
     RETURNS trigger
     LANGUAGE plpgsql
     AS $$
     BEGIN
     ...
     END;
     $$;`);
    await execute(connection, `DROP TRIGGER IF EXISTS printel_anaf_order_change_trigger ON "order";
     CREATE TRIGGER printel_anaf_order_change_trigger
     AFTER UPDATE ON "order"
     FOR EACH ROW
     EXECUTE PROCEDURE printel_anaf_handle_order_changes();`);
    await execute(connection, `CREATE OR REPLACE FUNCTION printel_anaf_handle_order_item_changes()
     RETURNS trigger
     LANGUAGE plpgsql
     AS $$
     DECLARE
     ...
     END;
     $$;`);
    await execute(connection, `DROP TRIGGER IF EXISTS printel_anaf_order_item_change_trigger ON order_item;
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION printel_anaf_handle_order_address_changes()
     RETURNS trigger
     LANGUAGE plpgsql
     AS $$
     BEGIN
     ...
     END;
     $$;`);
    await execute(connection, `DROP TRIGGER IF EXISTS printel_anaf_order_address_change_trigger ON order_address;
