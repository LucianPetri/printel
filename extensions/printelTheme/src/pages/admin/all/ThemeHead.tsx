...
export default function ThemeHead() {
  React.useEffect(() => {
    const replaceAdminLogo = () => {
      const adminLogoLink = document.querySelector<HTMLAnchorElement>(
        '.header .printel-admin-logo a'
      );
      if (adminLogoLink) {
      ...
      const existingBrand = loginForm.querySelector<HTMLElement>(
        '[data-printel-login-brand]'
      );
      if (!existingBrand) {
      ...
      const legacyLogo = loginForm.querySelector<HTMLElement>('div.flex.items-center.justify-center.mb-7');
      if (legacyLogo) {
  ...
  return (
    <>
      ...
      <link rel="preload" as="image" href={adminLogoSrc} />
